return {
  ...
  keys = {
    ...
    },
    {
      "<leader>/",
      function()
        Snacks.picker.grep({ cwd = vim.fn.getcwd(0) })
      end,
      desc = "Grep",
    },
    {
  ...
  init = function()
    local inlay_hints_state_file = vim.fs.joinpath(vim.fn.stdpath("state"), "inlay-hints.json")
    ...
    local function write_inlay_hints_state(enabled)
      vim.fn.mkdir(vim.fs.dirname(inlay_hints_state_file), "p")
      vim.fn.writefile({ vim.json.encode({ enabled = enabled }) }, inlay_hints_state_file)
    end
